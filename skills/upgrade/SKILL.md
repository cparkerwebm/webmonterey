...
4. Raise the site's wrangler to the floor this version was tested with - one `npm install` of
   one spec, nothing else touched; a site already at or above it is left alone
5. `webm sync` — re-materialize the fleet skills
6. `webm queue` — create the site's queues and wire them, when wrangler is logged in, and say
   what on this site now goes through the queue; otherwise it says so and the site keeps
   sending inline (`--no-queue` skips it)
7. `webm doctor` — the traps that fail silently
8. `npm run check` and `npm run build`

**Tell the person what the upgrade changed on this site**, from the command's own output: the
codemods that ran, what the toolchain step moved, and the queue outline - which mail and which
jobs now leave through the queue and which stay in the request. That outline is the record of
what a client site does differently after this upgrade.
